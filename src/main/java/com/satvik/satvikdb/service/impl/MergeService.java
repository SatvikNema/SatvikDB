...
public class MergeService {
  ...
  public void merge() throws IOException {
    List<DbFilePath> files =
        GeneralUtils.loadFilesSortedByAgeDesc(
            rootDir, indexFileStartsWith, dbFileStartsWith, "null");
  ...
  private void mergeFiles(List<DbFilePath> filesToMerge) throws IOException {
    List<String> dbFiles =
        filesToMerge.stream().map(DbFilePath::getDbFilePath).collect(Collectors.toList());
  ...
  private void mergeFilesHelper(List<RandomAccessFile> rafs) throws IOException {
    ...
    while (true) {
      ...
      for (RafEntry entry : entries) {
        ...
        if (key != null
            && minKey.compareTo(key)
                >= 0) { // if key is same, take this one, as the current file is latest
          minKey = key;
      ...
      dataToWrite.add(
          new KeyValuePair(
              minKey,
              GeneralUtils.getParsedValue(entryWithLeastKey.getCurrentLine().toCharArray())));
